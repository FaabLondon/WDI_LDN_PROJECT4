...
describe('DELETE /items/:id/reviews/:reviewId', () => {
  beforeEach(done => {
    Promise.all([
      Item.remove({}),
      User.remove({})
      ...
      .then(data => {
        ...
        reviewData = {
          ...
          user: data.user._id,
          createdAt: '2018-04-12T09:25:51.253Z',
          updatedAt: '2018-04-12T09:25:51.253Z'
        };
        //add the review the reviews array in user
        data.item.reviews.push(reviewData);
        data.item.save();
        reviewId = data.item.reviews[0]._id;
      })
      .then(done);
  });
